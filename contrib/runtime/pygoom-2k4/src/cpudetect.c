...
// System Instructions, Table 3-2: Effective family computation, page 120.
	...
	if(regs[0]>=0x80000006)
	{
		...
		printf("extended cache-info: %d\n",regs2[2]&0x7FFFFFFF);
		caps->cl_size  = regs2[2] & 0xFF;
	}
	printf("Detected cache-line size is %u bytes\n",caps->cl_size);
#if 0
...
static void check_os_katmai_support( void )
{
   ...
   printf( "Testing OS support for SSE2... " );
...
#else
   gCpuCaps.hasSSE = 0;
   printf( "No OS support for SSE, disabling to be safe.\n" );
#endif
#elif defined(WIN32)
...
#endif //__AMIGAOS4__
#endif /* SYS_DARWIN */
        printf("AltiVec %sfound\n", (caps->hasAltiVec ? "" : "not "));
#endif /* HAVE_ALTIVEC */

#ifdef ARCH_IA64
	printf("CPU: Intel Itanium\n");
#endif

#ifdef ARCH_SPARC
	printf("CPU: Sun Sparc\n");
#endif
...
#ifdef ARCH_PA_RISC
	printf("CPU: Hewlett-Packard PA-RISC\n");
#endif

#ifdef ARCH_S390
	printf("CPU: IBM S/390\n");
#endif

#ifdef ARCH_S390X
	printf("CPU: IBM S/390X\n");
#endif

#ifdef ARCH_VAX
	printf( "CPU: Digital VAX\n" );
#endif
}
#endif /* !ARCH_X86 */
