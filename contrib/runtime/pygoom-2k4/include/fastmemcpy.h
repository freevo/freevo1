...
#ifdef VERBOSE
#warning HAVE_MMX/MMX2/3DNOW/SSE/SSE2
#endif
...
static inline void * mem2agpcpy_pic(void * dst, const void * src, int bytesPerLine, int height, int dstStride, int srcStride)
{
	...
	void *retval=dst;
#ifdef VERBOSE
        printf ("mem2agpcpy_pic(%p, %p, %d, %d, %d, %d)\n", dst, src, bytesPerLine, height, dstStride, srcStride);
#endif
                                 ...
                                 int dstStride, int srcStride, int limit2width)
{
	...
	void *retval=dst;
#ifdef VERBOSE
        printf ("memcpy_pic(%p, %p, %d, %d, %d, %d, %d)\n", 
		dst, src, bytesPerLine, height, dstStride, srcStride, limit2width);
#endif
