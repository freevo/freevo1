    ...
    def _GCLMVideopass(self, passnr):
        ...
        if self.vcodec == "XViD" and (self.crop != None):
            if self.ana:
                ...
                if rounded > 7.5:
                    yscaled += 16
                if DEBUG: print "Rescaled, corrected for AR res is %sx%s" % (self.cropres[0], int(yscaled))
            ...
            if DEBUG: print "Rescaled, rounded yres is %sx%s" % (idealres[0], idealres[1])
...
class EncodingQueue:
    ...
    def _runQueue(self, line="", data=""):
        ...
        self.log.info("Started job %s, %s on PID %s" % (self.currentjob.idnr, status[self.currentjob.status], self.currentjob.pid))
