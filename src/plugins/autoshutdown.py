...
def shutdown_action(action=None):
	...
	if (action == Shutdown.RESTART_SYSTEM):
		...
		__cleanup_freevo()
		__syscall(config.RESTART_SYS_CMD, (config.AUTOSHUTDOWN_PRETEND == True))
		...
		__cleanup_freevo()
		__syscall(config.SHUTDOWN_SYS_CMD, (config.AUTOSHUTDOWN_PRETEND == True))
		# wait until the system halts/reboots
		while 1:
			time.sleep(1)
...
def __schedule_wakeup_and_shutdown():
	try:
		...
		cmd = "%s %s --settime %d" % (config.AUTOSHUTDOWN_NVRAM_CMD, config.AUTOSHUTDOWN_NVRAM_OPT, int(wakeup_utc_s))
		__syscall(cmd)
