...
def init():
    ...
    if hasattr(config, 'ROM_DRIVES'):
        ...
        if len(config.REMOVABLE_MEDIA) > 0:
...
def shutdown():
    ...
    """
    global im_thread
    if im_thread and im_thread.isAlive():
...
class autostart(plugin.DaemonPlugin):
    ...
    def __init__(self):
        ...
        """
        if not config.ROM_DRIVES:
            self.reason = 'No ROM_DRIVES configured in local_conf.py'
            return
...
class Identify_Thread(threading.Thread):
    ...
    def identify(self, media, force_rebuild=False):
        ...
        cds = media.get_drive_status()
        if cds == -1:
            #_debug_('cannot get status for drive %r' % (media.devicename))
            return
