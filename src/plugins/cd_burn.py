...
class BurnCDItem:
	...
	if self.item.filename:
	        print 'CDBURN : findFileFromItem() found item %s' %self.item.filename
...
class main_burn_thread(threading.Thread):
	...
	if self.token.burn_mode == "data_cd":
		...
		mkisofs_cmd = '%s -f -U -V "%s" -o %s %s' % (config.CDBURN_MKISOFS_PATH,self.token.volume_name,image_file,burn_list)
		...
		cdrecord_cmd = '%s -eject -v -driveropts=burnfree speed=%s dev=%s %s' % (config.CDBURN_CDRECORD_PATH,config.CDBURN_SPEED,config.CDBURN_DEV,image_file) 
		...
		os.unlink(image_file)
                ...
                growisofs_cmd = 'growisofs -use-the-force-luke -dvd-compat -Z /dev/dvd -V %s -dvd-video %s' % (self.token.name,self.token.dir)
