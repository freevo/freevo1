...
class MusicIPClient:
    ...
    def getGenres(self, filter=None, showCount=False):
        ...
        named filter when creating the list (default none) 1.6 showCount Prefix each
    ...
    def getMix(self, song=None, artist=None, album=None, mixgenre=None, mood=None, playlist=None, rejectsize=None,
        ...
        you may not mix different types in a single request. If no seeds are set, a
    ...
    def getSongs(self, album=None, artist=None, content=None, filter=None, extended=None):
        ...
        album=fullpath Only include songs from the named album. fullpath is any song
        from the album album=artistname@@albumname Only include songs from the named
    ...
    def getSongInfo(self, index=None, file=None):
        ...
        the field name, and the rest of the line is the field value. Some fields may
        not be present if there is no associated value. Current fields are: name,
...
class PluginInterface(plugin.ItemPlugin):
    ...
    def actions(self, item):
        ...
        if item.type == 'audio':
            ...
            items.append((self.file_play_all_by_artist, _('Songs From Same Artist'), \
    ...
    def file_play_all_by_artist(self, arg=None, menuw=None):
        ...
        #print '\n'.join(filenames)
        #items = [kaa.beacon.query(filename=f) for f in filenames]
        playlist = Playlist('Music by '+songInfo['artist'], playlist=filenames, display_type="audio", autoplay=True)
        playlist.browse(arg=arg, menuw=menuw)
