...
# File - Followed by a list of tuples containing text description and shell style
#        filter.
PROP_TYPE_FILE   = 'file'
...
class DesignerObject(object):
    """
    Base class for all designer objects.
    @ivar properties: A tuple of tuples containing a property name, text description
    property type and optionally additional type information.
    Property type is one of the following:
    PROP_TYPE_STRING (string) - can be followed by either a list of options or a function to call to
                                retrieve the list.
    PROP_TYPE_INT (int) - Followed by a tuple containing the lower and upper bounds.

    PROP_TYPE_FILE (file) - Followed by a list of tuples containing text description and a list of
                            shell style filters.
    PROP_TYPE_BOOL (bool) - True of false value

    PROP_TYPE_OPTION (option) - A fixed list of options one of which must be selected follows this.
...
class ImageObject(DialogChildObject):
    ...
    def __init__(self, node=None):
        ...
        self.properties = self.properties + (
                                             ('expr', 'Expression', PROP_TYPE_STRING),
                                             ('src', 'Source', PROP_TYPE_FILE, (('Image', ('*.png', '*.jpg')),)),
                                             ('src_is_expr', 'Source is Expression', PROP_TYPE_BOOL),
                                             ('scale', 'Scaling', PROP_TYPE_OPTION, ('noscale', 'horizontal','vertical','both','aspect'))
                                             )
        if node:
